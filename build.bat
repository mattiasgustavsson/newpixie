...
set WARNING_FLAGS=/Wall /wd 4514 /wd 4710 /wd 4711 /wd 4738 /wd 4820 /wd 5045 /WX
...
:param_init
	if "%1"=="amd64" ( set param=x64 ) else ( set param=x86 ) 
	goto :eof

:vsvars32	
	set VCINSTALLDIR=
	for /f "tokens=1,2*" %%a in ('reg query "HKLM\SOFTWARE\Wow6432Node\Microsoft\VisualStudio\SxS\VS7" /v "15.0" 2^>nul') do ( set "VCINSTALLDIR=%%cVC" )
	if defined VCINSTALLDIR  call :xp_toolset & call :param_init %1 & goto :vsvars32_done            
	set param=%1
	...
	:vsvars32_done
		...
		if exist "%VCINSTALLDIR%\Auxiliary\Build\vcvarsall.bat" call "%VCINSTALLDIR%\Auxiliary\Build\vcvarsall.bat" %param% 1>nul	
